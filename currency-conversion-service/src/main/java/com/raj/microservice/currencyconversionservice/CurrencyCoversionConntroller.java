package com.raj.microservice.currencyconversionservice;
...
@RestController
public class CurrencyCoversionConntroller {
	...
	public CurrencyConversionBean convertFeign(@PathVariable String from, @PathVariable String to,
		...
		// Feign Problem-1
		CurrencyConversionBean response = proxy.retieveExchangeValue(from, to);
		return new CurrencyConversionBean(response.getId(), from, to, response.getConversionMultiple(), quantity,
				quantity.multiply(response.getConversionMultiple()), response.getPort());
	}
